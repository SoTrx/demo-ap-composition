...
# Demo datasets seeded into moma-management so the Plan tabs' dataset picker has
# something to offer. A subdirectory on purpose: list_ap_files() globs ASSETS_DIR
# non-recursively, so these never show up as Analytical Patterns.
DATASETS_DIR = ASSETS_DIR / "datasets"

# The root node label of a moma Dataset graph.
DATASET_ROOT_LABEL = "sc:Dataset"

# Label ap-management puts on an operator node its magic operator generated.
MAGIC_OPERATOR_LABEL = "Magic_Operator"

# The dataset kinds that actually decide which operators may run against a dataset,
# mirroring ap-management's internal/dataset_compat.py. A dataset graph also carries
# structural labels (Data, Column, StructuredData…) that say nothing useful here, so
# these are what a picker should show first.
SIGNIFICANT_DATASET_KINDS = (
    "RelationalDatabase", "Table", "CsvSet", "CSV",
    "PdfSet", "PDF", "ImageSet", "Image", "VideoSet", "Video",
)


def summarize_dataset_kinds(kinds: list[str], limit: int = 3) -> list[str]:
    """The kinds worth showing for a dataset: the compatibility-bearing ones if the
    dataset has any, otherwise whatever it does have."""
    significant = [k for k in SIGNIFICANT_DATASET_KINDS if k in kinds]
    return (significant or list(kinds))[:limit]
...
def list_dataset_files() -> list[str]:
    return [str(path) for path in sorted(DATASETS_DIR.glob("*.json"))]
...
def ap_to_graphviz(ap_data: dict, status_by_id: dict[str, str] | None = None) -> str:
    ...
    for i, node in enumerate(nodes):
        ...
        elif MAGIC_OPERATOR_LABEL in labels:
            # A step no catalogued AP covered, generated by the planner's magic
            # operator. Worth telling apart from a catalogued one at a glance: it is
            # LLM-authored, and it only runs if the executor can resolve a
            # "magic-operator" service.
            color, shape = "plum", "box"
            name = f"\u2728 {name}"
