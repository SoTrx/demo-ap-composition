...
# Label ap-management's planner puts on the operator it generates for a step no
# catalogued AP covers (allow_magic_operator).
...
def list_ap_files() -> list[tuple[str, str, str]]:
    results = []
    for path in sorted(APS_DIR.glob("*.json")):
        try:
            with open(path) as f:
                data = json.load(f)
...
_DATASET_ROOT_LABEL = "sc:Dataset"


def list_dataset_files() -> list[dict]:
    """One ``{"name", "path", "id", "kinds", "description"}`` entry per dataset
    descriptor in ``assets/datasets``.

    ``id`` is the ``sc:Dataset`` root node's id — the id ap-management's planner
    takes in ``dataset_ids`` and fetches from moma. ``kinds`` are every other node
    label in the graph, which is what ap-management matches operators against
    (mirrors its ``DatasetSummary.from_dataset``). Finding which datasets are
    relevant to a task is a separate service's job — this only enumerates what
    is on disk. Files without a root node are skipped.
    """
    results = []
    for path in sorted(DATASETS_DIR.glob("*.json")):
        try:
            ...
            continue
        root = next((n for n in nodes if _DATASET_ROOT_LABEL in n.get("labels", [])), None)
        if not root:
            continue
        props = root.get("properties", {}) or {}
        kinds = list(dict.fromkeys(
            lbl for n in nodes for lbl in n.get("labels", []) if lbl != _DATASET_ROOT_LABEL))
        results.append({
